...
class ChatBloc extends Bloc<ChatEvent, ChatState> {
  ...
  Future<void> _onSend(SendMessage e, Emitter<ChatState> emit) async {
    final userMsg = ChatMessageModel(
        role: 'user', content: e.message, timestamp: DateTime.now());
    ...
    try {
      ...
      emit(ChatUpdated(
          ...
          suggestions: resp.suggestedQuestions,
          isLoading: false));
    } catch (err) {
      _history.removeLast();
  ...
  void _onClear(ClearHistory e, Emitter<ChatState> emit) {
    _history.clear();
